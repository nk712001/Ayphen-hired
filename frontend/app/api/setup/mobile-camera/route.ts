...
export async function POST(req: NextRequest) {
  try {
    ...
    if (connected && frameData) {
      ...
      // Store frame for dual-camera analysis during tests
      try {
        const { storeMobileFrame } = await import('@/lib/mobile-frame-store');
        storeMobileFrame(sessionId, frameData);
      } catch (error) {
        console.error('Failed to store frame for dual-camera analysis:', error);
      }
    }
