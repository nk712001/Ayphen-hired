...
interface RoleSpecificLoginFormProps {
  role: Role;
  title: string;
  subtitle: string;
}

export const RoleSpecificLoginForm = ({ role, title, subtitle }: RoleSpecificLoginFormProps) => {
    ...
    try {
      ...
      if (result?.error) {
        ...
        if (session?.user?.role) {
          ...
          // Fallback to role-specific redirect
          router.push(role === 'admin' ? '/admin/dashboard' : '/interviewer/dashboard');
        }
      }
    } catch (err) {
