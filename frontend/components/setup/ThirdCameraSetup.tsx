...
const ThirdCameraSetup: React.FC<ThirdCameraSetupProps> = ({
  ...
  // Get network IP on mount and regenerate QR code when IP changes
  useEffect(() => {
    getNetworkIp().then(() => {
      // Regenerate QR code after getting network IP
      if (mobileSessionId) {
        generateQRCode();
      }
    });
  ...
  // Regenerate QR code when network IP changes
  useEffect(() => {
    if (networkIp && mobileSessionId) {
      console.log('[QR Code] Network IP updated, regenerating QR code');
      generateQRCode();
    }
  }, [networkIp, mobileSessionId]);
