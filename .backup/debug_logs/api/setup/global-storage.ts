...
class MobileStorage {
  ...
  private initialize() {
    ...
    console.log('Mobile storage singleton initialized');
    ...
    setInterval(() => {
      ...
      if (cleanupCount > 0) {
        console.log(`Cleaned up ${cleanupCount} stale mobile camera frames`);
      }
    }, 60000); // Run cleanup every minute
  }
  ...
  public addTestData(sessionId: string): void {
    ...
    console.log(`Added test data for session: ${sessionId}`);
    console.log(`Current connections: ${Object.keys(this.connections).join(', ')}`);
    console.log(`Current frames: ${Object.keys(this.frames).join(', ')}`);
  }
  ...
  public updateConnection(sessionId: string, connected: boolean, streamUrl: string | null): void {
    ...
    console.log(`Updated connection for session: ${sessionId}`);
    console.log(`Current connections: ${Object.keys(this.connections).join(', ')}`);
  }
  ...
  public updateFrame(sessionId: string, frameData: string): void {
    ...
    if (this.frames[sessionId].frameCount! % 10 === 0) {
      console.log(`Updated frame #${this.frames[sessionId].frameCount} for session: ${sessionId}`);
      console.log(`Current frames: ${Object.keys(this.frames).join(', ')}`);
    }
  }
}
